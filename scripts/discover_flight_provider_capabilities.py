...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--require-no-secret-output',action='store_true'); args=ap.parse_args()
    report={}
    for provider,names in PROVIDERS.items():
        flag=truthy(os.getenv(names[0])); direct=bool(os.getenv(names[1]))
        report[provider]={'configured':bool(flag or direct),'discovery_source':'boolean_flag' if flag else ('environment_presence' if direct else 'none'),'secret_value_exposed':False}
