"""Skywatcher maintenance layer: detection, export-dir + threshold checks."""
...
def test_missing_federation_json_is_critical(tmp_path):
    ...
    findings = detect.detect_missing_required_files("skywatcher-pr", tmp_path, state)
...
def test_audit_clean_repo_not_blocked(tmp_path):
    (tmp_path / "exports" / "federation").mkdir(parents=True)
    _write_threshold(
        tmp_path,
        "exports/satim_calibration/c1/summary.json",
