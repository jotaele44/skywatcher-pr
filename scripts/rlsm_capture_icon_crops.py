...
def _capture_one(
    ...
    *,
    ...
    base = {
        ...
        "source_rel_path": str(row["rel_path"]),
    ...
    try:
        ...
        return {
            **base,
            "capture_status": "ok",
            "capture_error": None,
            "crop_rel_path": output.relative_to(REPO).as_posix(),
...
def _write_manifest(conn: sqlite3.Connection, path: Path) -> int:
    rows = conn.execute(
        """SELECT a.icon_id, a.screenshot_id, a.source_sha256, a.crop_rel_path,
                  ...
                  i.bbox_w, i.bbox_h
           FROM icon_artifacts a
           JOIN icon_observations i ON i.icon_id=a.icon_id
           ORDER BY a.icon_id"""
    ).fetchall()
...
def run(
    ...
    *,
    ...
    sql = """SELECT i.icon_id, i.screenshot_id, i.bbox_x, i.bbox_y,
             ...
             FROM icon_observations i
             JOIN screenshots s ON s.screenshot_id=i.screenshot_id
             ORDER BY i.icon_id"""
    if limit:
        ...
        with conn:
            conn.execute(
                ...
                (
                    ...
                    result.get("crop_rel_path"),
                    result.get("crop_sha256"),
                    result["capture_status"],
                    result.get("capture_error"),
                    METHOD,
                    observed_at,
                ),
            )
