...
def make_readiness(name: str, stamp: str) -> dict[str, Any]:
    ...
    for d in DIMS:
        c = (
            50
            if d
            in {"operator_report", "offline_dashboard", "blocker_tracking", "release_packaging"}
            else 25
        )
        dims.append(
            {
                ...
                "completion_pct": c,
                "status": "yellow" if c >= 50 else "unknown",
                "required": True,
                "blockers": [],
                "evidence_refs": [],
            }
    ...
    return {
        "schema_version": "federation.readiness.v1",
        "repo": name,
        "generated_at": stamp,
        "overall_status": "red" if overall < 40 else "yellow",
...
def make_blockers(name: str, stamp: str) -> dict[str, Any]:
    return {
        "schema_version": "federation.blockers.v1",
        "repo": name,
        "generated_at": stamp,
        "blockers": [
            {
                "id": "BLOCKER-REPO-INTEGRATION",
                "severity": "medium",
                "status": "open",
                "scope": "export",
                "title": "Connect offline scaffold to repo-native artifacts",
                "impact": "Offline contract exists; repo-specific outputs still need to be mapped into readiness and evidence files.",
                "resolution": "Replace scaffold defaults with native export, validation, and evidence outputs.",
                "evidence_refs": [],
            }
        ],
    }
        ...
        "sources": [
            {
                ...
                "cadence": "unknown",
                "status": "candidate",
                "notes": "Scaffold placeholder; replace with repo-native source registry entries.",
            }
...
def refresh_manifest(path: Path, name: str) -> None:
    r = jread(path / "readiness.json", {})
    b = jread(path / "blockers.json", {"blockers": []})
    s = jread(path / "sources.json", {"sources": []})
    files = [
        {"path": p.name, "type": p.suffix.lstrip(".") or "file", "sha256": sha(p)}
    ...
    m = {
        ...
        "repo": name,
        "node_type": "producer",
        "package_id": f"{name}_{now()}",
        "generated_at": now(),
        "commit_sha": git(["rev-parse", "HEAD"]),
        "branch": git(["rev-parse", "--abbrev-ref", "HEAD"]),
        "data_mode": "diagnostic_seed",
        "production_status": "diagnostic",
        "offline_ready": True,
        "localhost_required": False,
        "summary": {
            "records": 0,
            "sources": len(s.get("sources", [])),
            "blockers_open": sum(1 for x in b.get("blockers", []) if x.get("status") == "open"),
            "critical_blockers_open": 0,
            "evidence_items": 4,
            "overall_completion_pct": r.get("overall_completion_pct", 0),
        },
        "files": files,
        "gates": {
            "tests": "unknown",
            ...
            "offline_dashboard_generated": "pass" if (path / "dashboard.html").exists() else "fail",
...
def export_cmd(argv: list[str] | None = None) -> int:
    path, name, stamp = out(argv), repo(), now()
    path.mkdir(parents=True, exist_ok=True)
    (path / "artifacts").mkdir(exist_ok=True)
    r, b, s = make_readiness(name, stamp), make_blockers(name, stamp), make_sources(name, stamp)
    jwrite(path / "readiness.json", r)
    jwrite(path / "blockers.json", b)
    jwrite(path / "sources.json", s)
...
def dashboard_cmd(argv: list[str] | None = None) -> int:
    path, name = out(argv), repo()
    r = jread(path / "readiness.json", make_readiness(name, now()))
    b = jread(path / "blockers.json", make_blockers(name, now()))
    s = jread(path / "sources.json", make_sources(name, now()))
    dim_rows = "".join(
        f"<tr><td>{html.escape(x['id'])}</td><td>{html.escape(x['status'])}</td><td>{x['completion_pct']}%</td></tr>"
        for x in r.get("dimensions", [])
    )
    block_rows = "".join(
        f"<tr><td>{html.escape(x['id'])}</td><td>{html.escape(x['severity'])}</td><td>{html.escape(x['status'])}</td><td>{html.escape(x['title'])}</td></tr>"
        for x in b.get("blockers", [])
    )
    src_rows = "".join(
        f"<tr><td>{html.escape(x['source_id'])}</td><td>{html.escape(x['name'])}</td><td>{html.escape(x['category'])}</td><td>{html.escape(x['status'])}</td></tr>"
...
def validate_cmd(argv: list[str] | None = None) -> int:
    ...
    req = [
        ...
        "operator_report.md",
        "dashboard.html",
    ] + ([] if ns.allow_unpackaged else ["package.sha256"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", choices=["export", "dashboard", "package", "validate"])
    ns, rest = parser.parse_known_args()
    raise SystemExit(run(ns.command, rest))
