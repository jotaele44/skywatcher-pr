...
# NOTE: openpyxl is imported lazily inside parse() so that modules which only
# need the lightweight helpers (e.g. fr24.todays_batch) can import this module
# without requiring the openpyxl dependency to be installed.
...
def parse(xlsx_path: Path) -> list[dict]:
    import openpyxl  # lazy: only required when actually parsing a workbook
