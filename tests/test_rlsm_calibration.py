...
def test_empirical_calibration_pass_is_bound_to_exact_corpus(
    ...
    monkeypatch.setattr(calibration, "_ocr_case", fake_ocr_case)
...
def test_corpus_change_invalidates_prior_calibration(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        "_ocr_case",
        lambda path: (
            {"aircraft_card": "N407PR"}
            if path.name == "positive.png"
            else {"aircraft_card": "NO AIRCRAFT"},
            {"aircraft_card": 99.0},
        ),
    )
    calibration.run(
    ...
    try:
        ...
        assert row == ("OPEN", new_cert["corpus_digest"])
    finally:
        conn.close()
