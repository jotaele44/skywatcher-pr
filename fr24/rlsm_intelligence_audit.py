...
def audit_accounting(
    ...
    missing_on_disk = sorted(
        rel_path for rel_path in db_paths if rel_path not in disk and "_missing/" not in rel_path
    )
...
def audit_ocr_integrity(conn: sqlite3.Connection) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    strict_runs = _count(
        conn, "SELECT COUNT(*) FROM processing_runs WHERE run_kind='ocr_strict_parallel'"
    )
    errors: list[dict[str, Any]] = []
    errors.extend(
        {
            "kind": "missing_ocr_receipt",
            "severity": "high",
            "screenshot_id": r[0],
            "rel_path": r[1],
            "ocr_status": r[2],
    ...
    errors.extend(
        {
            "kind": "failed_ocr_without_error",
            "severity": "high",
            "obs_id": r[0],
            "screenshot_id": r[1],
            "zone": r[2],
        }
        for r in failed_without_error
    )
    errors.extend(
        {
            "kind": "screenshot_marked_ok_with_failed_zone",
            "severity": "high",
            "screenshot_id": r[0],
            "rel_path": r[1],
            "failed_zones": r[2],
        }
        for r in ok_with_latest_failure
    )
    errors.extend(
        {
            "kind": "partial_status_without_failed_zone",
            "severity": "medium",
            "screenshot_id": r[0],
            "rel_path": r[1],
        }
        for r in partial_without_failure
    )
    errors.extend(
        {
            "kind": "completed_run_with_unaccounted_inputs",
            "severity": "high",
            "run_id": r[0],
            "run_kind": r[1],
            "n_inputs": r[2],
        ...
        for r in incomplete_completed_runs
    )
    errors.extend(
        {
            "kind": "processing_run_left_in_progress",
            "severity": "medium",
            "run_id": r[0],
            "run_kind": r[1],
            "started_at": r[2],
    ...
    silent_failure_count = (
        ...
        + len(failed_without_error)
        + len(ok_with_latest_failure)
        + len(partial_without_failure)
        + len(incomplete_completed_runs)
    )
    status_counts = dict(
        conn.execute("SELECT ocr_status, COUNT(*) FROM screenshots GROUP BY ocr_status").fetchall()
    )
    return (
        {
...
def _optional_count(conn: sqlite3.Connection, table: str, where: str = "") -> int:
    return (
        _count(conn, f"SELECT COUNT(*) FROM {table} {where}") if _table_exists(conn, table) else 0
    )
...
def audit_capabilities(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    cv_tracks = _optional_count(
        conn, "flight_track_features", "WHERE bbox_x IS NOT NULL AND confidence>=0.6"
    )
    ...
    return {
        ...
        "ocr_text_labels": {
            "status": "implemented_unverified" if labels else "implemented_no_output",
            "labeled_pins": labels,
        },
        "unlabeled_visual_candidates": {
            "status": "optional_operator_run" if unlabeled == 0 else "implemented",
            "candidates": unlabeled,
        },
        "aircraft_metadata": {
            "status": "implemented_unverified" if aircraft else "implemented_no_output",
            "observations": aircraft,
        },
        "flight_path": {
            "status": "implemented_unverified" if tracks else "implemented_no_output",
            "observations": tracks,
            "pixel_derived": cv_tracks,
            "heuristic_or_absent": max(0, tracks - cv_tracks),
        },
        "frame_classification": {
            "status": "implemented" if frame_rows else "operator_run_required",
            "rows": frame_rows,
            "coverage_percent": _pct(frame_rows, ingest_ok),
        },
        "map_state": {
            "status": "layout_only_no_geolocation" if map_rows else "operator_run_required",
            "rows": map_rows,
        },
        "gui_artifacts": {
            "status": "implemented" if gui_rows else "operator_run_required",
            "rows": gui_rows,
        },
        "icons": {
            "status": "captured_with_failures"
            if icon_failures
            else "captured"
            if icons and icon_artifacts == icons
            else "detection_only"
            if icons
            else "implemented_no_output",
            "detected": icons,
            "artifacts": icon_artifacts,
            "artifact_failures": icon_failures,
        },
        "georeferencing": {
            "status": "anchors_only_unverified" if geo_anchors else "unsupported",
            "anchors": geo_anchors,
        },
        "field_provenance": {
            "status": "implemented" if provenance_fields else "operator_run_required",
...
CORE_PROVENANCE_FIELDS: dict[str, tuple[str, str, tuple[str, ...]]] = {
    "labeled_pin": (
        "labeled_pins",
        "pin_id",
        ("raw_label", "normalized_label", "bbox_x", "bbox_y", "bbox_w", "bbox_h", "confidence"),
    ),
    ...
    "icon_observation": (
        "icon_observations",
        "icon_id",
        ("bbox_x", "bbox_y", "bbox_w", "bbox_h", "ahash", "cluster_id", "icon_class", "confidence"),
    ),
    "frame_observation": (
        "frame_observations",
        "frame_obs_id",
        ("frame_type", "provider", "orientation", "confidence"),
    ),
    "map_state": (
    ...
    "icon_artifact": (
        "icon_artifacts",
        "icon_artifact_id",
        ("source_sha256", "crop_rel_path", "crop_sha256", "capture_status", "capture_error"),
    ),
}
...
def audit_provenance(conn: sqlite3.Connection) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if not _table_exists(conn, "extraction_field_provenance"):
        return (
            {
                "expected_non_null_core_fields": 0,
                "provenanced_core_fields": 0,
                "missing_core_fields": 0,
                "coverage_percent": 0.0,
                "complete": False,
                "status": "table_missing",
            },
            [{"kind": "field_provenance_table_missing", "severity": "high"}],
        )
    expected = missing = 0
    ...
    for object_type, (table, primary_key, fields) in CORE_PROVENANCE_FIELDS.items():
        ...
        by_object[object_type] = {
            "expected": object_expected,
            "missing": object_missing,
            "provenanced": object_expected - object_missing,
        }
    return (
        {
            "expected_non_null_core_fields": expected,
            "provenanced_core_fields": expected - missing,
            "missing_core_fields": missing,
            "coverage_percent": _pct(expected - missing, expected) if expected else 0.0,
...
def audit_geolocation(conn: sqlite3.Connection) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if not _table_exists(conn, "map_state_observations"):
        return (
            {
                "map_state_rows": 0,
                "unsupported_rows_with_coordinates": 0,
                "complete": False,
                "status": "table_missing",
            },
            [{"kind": "map_state_table_missing", "severity": "high"}],
        )
    rows = conn.execute(
    ...
    errors = [
        {
            "kind": "unsupported_geolocation_has_coordinates",
            "severity": "critical",
            "map_state_id": r[0],
            "screenshot_id": r[1],
            "center_lat": r[2],
    ...
    return (
        {
            "map_state_rows": _count(conn, "SELECT COUNT(*) FROM map_state_observations"),
            "unsupported_rows_with_coordinates": len(rows),
            "complete": len(rows) == 0,
        },
        errors,
    )
...
def _resolve_gold_screenshot(
    conn: sqlite3.Connection, row: dict[str, Any]
) -> tuple[int, str] | None:
    if row.get("screenshot_id") is not None:
        found = conn.execute(
            "SELECT screenshot_id, sha256 FROM screenshots WHERE screenshot_id=?",
            (row["screenshot_id"],),
        ).fetchone()
        ...
        found = conn.execute(
            "SELECT screenshot_id, sha256 FROM screenshots WHERE filename=? ORDER BY screenshot_id LIMIT 1",
            (row["filename"],),
        ).fetchone()
    else:
        return None
...
def _label_values(items: Any) -> set[str]:
    ...
    for item in items:
        text = (
            ...
            else str(item.get("text") or item.get("label") or "")
...
def evaluate_gold(
    ...
    try:
        ...
        return (
            ...
            [{"kind": "gold_sample_invalid", "severity": "high", "error": str(exc)}],
        )
    if not rows:
        return (
            {
    ...
    for index, gold_row in enumerate(rows):
        ...
        if resolved_row is None:
            ...
            errors.append(
                {
                    "kind": "gold_screenshot_unresolved",
                    "severity": "high",
                    "gold_index": index,
                    "identity": {
                        k: gold_row.get(k)
                        for k in ("screenshot_id", "screenshot_sha256", "filename")
                        if gold_row.get(k) is not None
                    },
                }
            )
            continue
        ...
        predicted_labels = {
            normalize_label(str(row[0] or ""))
            for row in predicted_rows
            if normalize_label(str(row[0] or ""))
        }
        tp += len(expected_labels & predicted_labels)
        fp += len(predicted_labels - expected_labels)
        fn += len(expected_labels - predicted_labels)
        expected_by_text = {
            normalize_label(str(item.get("text") or item.get("label") or "")): item
            for item in gold_row.get("labels", [])
            if isinstance(item, dict)
        }
        ...
        if expected_frame is not None and _table_exists(conn, "frame_observations"):
            predicted = conn.execute(
                "SELECT frame_type FROM frame_observations WHERE screenshot_id=? ORDER BY frame_obs_id DESC LIMIT 1",
                (sid,),
            ).fetchone()
            frame_total += 1
        ...
        if isinstance(expected_track, dict) and expected_track.get("path_shape") is not None:
            predicted = conn.execute(
                "SELECT path_shape FROM flight_track_features WHERE screenshot_id=? ORDER BY track_feat_id DESC LIMIT 1",
                (sid,),
            ).fetchone()
            track_total += 1
        ...
        if isinstance(expected_aircraft, dict):
            predicted = conn.execute(
                "SELECT registration, callsign, aircraft_type, altitude_ft, speed_kt, heading_deg, operator_text FROM aircraft_observations WHERE screenshot_id=? ORDER BY aircraft_obs_id DESC LIMIT 1",
                (sid,),
            ).fetchone()
            for field_index, field in enumerate(
                ...
                aircraft_correct += int(
                    str(predicted_value or "").casefold()
                    == str(expected_aircraft[field] or "").casefold()
                )
    ...
    metrics = {
        ...
        "label_metrics": {
            ...
            "mean_bbox_iou": round(sum(label_iou_values) / len(label_iou_values), 6)
            if label_iou_values
            else None,
        },
        ...
        "aircraft_field_accuracy": round(aircraft_correct / aircraft_total, 6)
        if aircraft_total
        else None,
    }
...
def build_structured_sample(conn: sqlite3.Connection, path: Path, limit: int = 25) -> int:
    rows = conn.execute(
        "SELECT screenshot_id, sha256, filename, rel_path, filename_ts, width, height, ingest_status, ocr_status FROM screenshots ORDER BY screenshot_id LIMIT ?",
        (max(0, limit),),
    ).fetchall()
        ...
        for row in rows:
            ...
            record["labels"] = [
                ...
                for i in conn.execute(
                    "SELECT pin_id, raw_label, normalized_label, bbox_x, bbox_y, bbox_w, bbox_h, centroid_x, centroid_y, pin_type_guess, confidence FROM labeled_pins WHERE screenshot_id=? ORDER BY pin_id",
                    (sid,),
                )
            ]
            record["aircraft"] = [
                ...
                for i in conn.execute(
                    "SELECT aircraft_obs_id, registration, callsign, aircraft_type, altitude_ft, speed_kt, heading_deg, operator_text, identity_status, confidence FROM aircraft_observations WHERE screenshot_id=? ORDER BY aircraft_obs_id",
                    (sid,),
                )
            ]
            record["tracks"] = [
                ...
                for i in conn.execute(
                    "SELECT track_feat_id, path_shape, has_loop, has_orbit, has_hover, has_gap, track_length_px, bbox_x, bbox_y, bbox_w, bbox_h, confidence FROM flight_track_features WHERE screenshot_id=? ORDER BY track_feat_id",
                    (sid,),
                )
            ]
            if _table_exists(conn, "frame_observations"):
                value = conn.execute(
                    "SELECT frame_type, provider, orientation, confidence, review_status FROM frame_observations WHERE screenshot_id=? ORDER BY frame_obs_id DESC LIMIT 1",
                    (sid,),
                ).fetchone()
                record["frame"] = list(value) if value else None
            if _table_exists(conn, "map_state_observations"):
                value = conn.execute(
                    "SELECT viewport_x, viewport_y, viewport_w, viewport_h, center_lat, center_lon, zoom, bearing_deg, geolocation_status, confidence FROM map_state_observations WHERE screenshot_id=? ORDER BY map_state_id DESC LIMIT 1",
                    (sid,),
                ).fetchone()
            ...
            if _table_exists(conn, "icon_observations"):
                record["icons"] = [
                    ...
                    for i in conn.execute(
                        "SELECT icon_id, pin_id, bbox_x, bbox_y, bbox_w, bbox_h, cluster_id, icon_class, confidence FROM icon_observations WHERE screenshot_id=? ORDER BY icon_id",
                        (sid,),
                    )
                ]
            if _table_exists(conn, "gui_artifact_observations"):
                record["gui_artifacts"] = [
                    {
                        "gui_artifact_id": i[0],
                        "artifact_type": i[1],
                        "bbox": list(i[2:6]),
                        "raw_text": i[6],
                        "confidence": i[7],
                        "extraction_status": i[8],
                    }
                    for i in conn.execute(
                        "SELECT gui_artifact_id, artifact_type, bbox_x, bbox_y, bbox_w, bbox_h, raw_text, confidence, extraction_status FROM gui_artifact_observations WHERE screenshot_id=? ORDER BY gui_artifact_id",
                        (sid,),
                    )
                ]
...
def build_gates(
    ...
    frame_complete = (
        capabilities["frame_classification"]["rows"] == capabilities["ingestion"]["screenshots"]
    )
    ...
    label_gate = (
        "PASS"
        if gold.get("status") == "ready" and label_recall is not None and label_recall >= 0.98
        else "FAIL"
        if gold.get("status") == "ready"
        else "BLOCKED"
    )
    return {
        "screenshot_accounting_100": _gate(
            "PASS" if accounting["complete"] else "FAIL",
            {
                "coverage_percent": accounting["coverage_percent"],
                "unregistered": accounting["disk_files_absent_from_database"],
                "missing_on_disk": accounting["database_files_absent_from_disk"],
            },
        ),
        "no_silent_failures": _gate(
            "PASS" if ocr["complete"] else "FAIL",
            {"silent_failure_count": ocr["silent_failure_count"]},
        ),
        "frame_accounting_100": _gate(
            "PASS" if frame_complete else "FAIL",
            {
                "frame_rows": capabilities["frame_classification"]["rows"],
                "ingest_ok": capabilities["ingestion"]["screenshots"],
            },
        ),
        "icon_capture_complete": _gate(
            "PASS" if icon_complete else "FAIL",
            {
                "detected": icons["detected"],
                "artifacts": icons["artifacts"],
                "failures": icons["artifact_failures"],
            },
        ),
        "no_unsupported_geolocation": _gate(
            "PASS" if geolocation["complete"] else "FAIL",
            {"violations": geolocation["unsupported_rows_with_coordinates"]},
        ),
        "field_level_provenance_100": _gate(
            "PASS" if provenance["complete"] else "FAIL",
            {
                "coverage_percent": provenance["coverage_percent"],
                "missing_core_fields": provenance["missing_core_fields"],
            },
        ),
        "location_label_recall_gte_0_98": _gate(
            label_gate,
            {
...
def _markdown(report: dict[str, Any]) -> str:
    ...
    for name in REQUIRED_GATES:
        ...
        lines.append(
            f"| `{name}` | **{gate['status']}** | `{json.dumps(gate['evidence'], sort_keys=True)}` |"
        )
    lines.extend(
        [
            "",
    ...
    if label_metrics:
        lines.extend(
            [
                f"- Label precision: `{label_metrics.get('precision')}`",
                f"- Label recall: `{label_metrics.get('recall')}`",
                f"- Label F1: `{label_metrics.get('f1')}`",
            ]
        )
    lines.extend(
        [
            ...
            "## Error ledger",
            "",
            f"- Total findings: **{report['error_count']:,}**",
            f"- Machine-readable ledger: `{report['outputs']['errors']}`",
            "",
            "A blocked gate is not a pass. The ≥98% label-recall requirement cannot be certified until the 300-frame annotated gold sample is present and fully resolvable.",
            "",
        ]
    )
    return "\n".join(lines)
...
def run(
    *,
    ...
    certification = (
        "PASS"
        if all(gates[name]["status"] == "PASS" for name in REQUIRED_GATES)
        else "BLOCKED"
        if any(gates[name]["status"] == "BLOCKED" for name in REQUIRED_GATES)
        else "FAIL"
    )
    ...
    paths = {
        "json": output_base / OUT_JSON.name,
        "markdown": output_base / OUT_MD.name,
        "capabilities": output_base / OUT_CAPABILITIES.name,
        "errors": output_base / OUT_ERRORS.name,
        "structured_sample": output_base / OUT_SAMPLE.name,
    }
    ...
    report = {
        ...
        "inputs": {
            ...
            "corpus_root": corpus_root.as_posix(),
            "gold_sample": gold_path.as_posix(),
        },
